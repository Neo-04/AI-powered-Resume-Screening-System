...
def _seed(monkeypatch, resume_skills, required_skills):
    resume = ParsedResume(resume_id="r1", skills=resume_skills, degree="B.Tech",
                          branch="Computer Science", education=["B.Tech Computer Science"])
    jd = ParsedJD(jd_id="j1", required_skills=required_skills, preferred_qualification=[],
...
def _semantic(monkeypatch, skill_hits=()):
    monkeypatch.setattr(semantic_matcher, "match_skills", lambda *a, **k: list(skill_hits))
    ...
    monkeypatch.setattr(semantic_matcher, "experience_matches", lambda *a, **k: (False, 0.0))
...
# 2. alias / normalized
@pytest.mark.parametrize("resume_skill,jd_skill", [
    ("Machine Learning", "ML"), ("NodeJS", "Node.js"), ("Postgres", "PostgreSQL"), ("JS", "JavaScript"),
])
...
@pytest.mark.parametrize("resume_skill,jd_skill", [
    ...
    ("PyTorch", "TensorFlow"), ("MySQL", "PostgreSQL"), ("React", "Angular"),
])
...
def test_related_match_uses_partial_weight(monkeypatch):
    _seed(monkeypatch, ["Python", "SQL", "Docker", "JavaScript"], ["Python", "SQL", "Docker", "TypeScript"])
    _semantic(monkeypatch)
    r = matching_service.run_match("r1", "j1")
...
def test_semantic_match_uses_partial_weight(monkeypatch):
    _seed(monkeypatch, ["Python", "SQL", "Docker", "Containerization"], ["Python", "SQL", "Docker", "Podman"])
...
def test_semantic_matcher_does_not_reuse_either_side():
    ...
    accepted = matching_service._consume_pairs([(j, r) for j, r, _ in pairs], matched, missing, additional)
    assert accepted == 1
...
def test_consistent_across_repeated_scoring(monkeypatch):
    ...
    assert len({(tuple(r.matched_skills), tuple(r.missing_skills),
                 tuple(r.additional_skills), r.match_score) for r in results}) == 1
...
def test_related_skills_are_not_aliases():
    assert skills_normalizer.canonical_skill("JavaScript") != skills_normalizer.canonical_skill("TypeScript")
    ...
    assert not skills_normalizer.are_related_skills("React", "ReactJS")  # identical, not related
