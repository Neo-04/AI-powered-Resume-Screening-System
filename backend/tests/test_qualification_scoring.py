...
@pytest.mark.parametrize("degree,branch,jd_text,branch_pts", [
...
@pytest.mark.parametrize("degree,jd_text,degree_pts", [
    ("MBA", "MBA required", 9),
...
@pytest.mark.parametrize("degree,branch,jd_text", [
...
@pytest.mark.parametrize("branch,jd_text", [
    ("Information Technology", "B.Tech in Computer Science"),
    ("Electronics and Communication Engineering", "B.Tech in Electrical Engineering"),
...
@pytest.mark.parametrize("degree,jd_text,expected", [
...
def test_score_always_within_weight_across_domains():
    degrees = ["B.Tech", "B.E.", "B.Sc", "BCA", "B.Com", "B.A.", "M.Tech", "M.Sc", "MCA", "MBA", ""]
    branches = ["Computer Science", "Information Technology", "Electrical Engineering",
                "Mechanical Engineering", "Civil Engineering", "Mathematics", "Statistics",
                "Finance", "Commerce", "Psychology", ""]
    for jd_text in ["B.Tech in Computer Science", "MBA in Finance", "Bachelor's degree", "Nothing stated"]:
        for d in degrees:
            for b in branches:
...
def test_no_branch_specific_conditions_in_matcher_source():
    ...
    for literal in ("Electrical", "Computer Science", "Information Technology",
                    "Software Engineering", "Electronics", "Mechanical"):
        assert literal not in source


def test_components_sum_to_match_score():
    scores = score_calculator.calculate_scores(
        skill_credit=2, required_skill_count=4, experience_match=True,
        qualification_score=9, soft_credit=2, jd_keyword_count=5,
    )
    assert scores.qualification == 9
    assert scores.skill + scores.experience + scores.qualification + scores.soft == scores.total


def test_qualification_component_not_scaled():
    scores = score_calculator.calculate_scores(
        skill_credit=4, required_skill_count=4, experience_match=True,
        qualification_score=15, soft_credit=5, jd_keyword_count=5,
    )
    assert (scores.skill, scores.experience, scores.qualification, scores.soft) == (50, 30, 15, 5)
    assert scores.total == 100


def _seed(monkeypatch, jd_text, degree, branch):
    quals = qualifications.extract_qualifications(jd_text)
    resume = ParsedResume(resume_id="r1", skills=["Python"], education=[f"{degree} {branch}"],
                          degree=degree, branch=branch)
    jd = ParsedJD(jd_id="j1", required_skills=["Python"], preferred_qualification=quals,
                  experience="0-2 years", keywords=[])
    ...
    monkeypatch.setitem(jd_service._jd_store, "j1",
                        JobDescription(jd_id="j1", filename="jd.txt", char_count=len(jd_text), text=jd_text))
...
def _no_semantic(monkeypatch):
    ...
    monkeypatch.setattr(semantic_matcher, "qualification_matches", lambda *a, **k: (False, 0.0))


@pytest.mark.parametrize("jd_text,degree,branch,expected", [
...
def test_response_score_matches_assessment_across_domains(monkeypatch, jd_text, degree, branch, expected):
    ...
    assert r.skill_score + r.experience_score + r.qualification_score + r.soft_skill_score == r.match_score
...
def test_semantic_runs_only_when_stated_requirement_scores_zero(monkeypatch):
    ...
    monkeypatch.setattr(semantic_matcher, "match_skills", lambda *a, **k: [])
    monkeypatch.setattr(semantic_matcher, "match_keywords", lambda *a, **k: [])
    monkeypatch.setattr(semantic_matcher, "qualification_matches",
...
def test_response_schema_unchanged(monkeypatch):
    ...
    assert set(result.model_dump().keys()) == {
        "match_score", "skill_score", "experience_score", "qualification_score",
        "soft_skill_score", "matched_skills", "missing_skills", "additional_skills",
        "qualification_match", "matching_reason", "experience_match",
