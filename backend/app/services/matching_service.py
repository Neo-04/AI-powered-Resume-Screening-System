...
def _parsed_resume(resume_id: str):
    return resume_parser.structured_resume_store.get(
        resume_id
    ) or resume_parser.parse_resume(resume_id)
...
def run_match(resume_id: str, jd_id: str) -> MatchResult:
    ...
    partial_skill_units = _consume_pairs(
        related_pairs, matched_skills, missing_skills, additional_skills
    )
    ...
    experience_match = experience_matcher.match_experience(
        resume.experience_years, jd.experience
    )
    descriptive_experience = experience_matcher.is_descriptive_requirement(
        jd.experience
    )
    ...
    if semantic_config.SEMANTIC_ENABLED:
        try:
            skill_hits = semantic_matcher.match_skills(
            ...
            keyword_hits = semantic_matcher.match_keywords(
                unmatched_keywords, resume_text
            )
            ...
            if assessment.requirement_mentioned and assessment.mentioned_score == 0:
                resume_qual_text = " ".join(
                    filter(None, [resume.degree, resume.branch, *resume.education])
                )
                ok, _ = semantic_matcher.qualification_matches(
                    resume_qual_text, jd.preferred_qualification
                )
                if ok:
                    ...
                    qualification_points = min(
                        ...
                        + semantic_config.SEMANTIC_QUALIFICATION_SCORE,
                    )
            ...
            logger.warning(
                "Semantic layer unavailable; using rule-based results only.",
                exc_info=True,
            )

    skill_credit = (
        exact_skill_units + semantic_config.SEMANTIC_MATCH_WEIGHT * partial_skill_units
    )
    soft_credit = (
        rule_keyword_covered
        + semantic_config.SEMANTIC_MATCH_WEIGHT * semantic_keyword_units
    )
